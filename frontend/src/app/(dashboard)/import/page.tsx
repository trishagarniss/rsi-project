...
export default function DataImportPage() {
  ...
  useEffect(() => {
    ...
    async function loadStudents() {
      try {
        setIsLoading(true);
        const res = await get<{ status: string; data: Student[] }>('/students/?skip=0&limit=100');
        if (isMounted) {
    ...
    <div className="space-y-6 ">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="flex-1 w-full flex items-center gap-3 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus-within:border-asgard-primary focus-within:ring-2 focus-within:ring-asgard-primary/20 transition-all">
          ...
          <input type="text" placeholder="Cari nama siswa..." className="w-full bg-transparent border-none focus:outline-none text-sm font-bold text-slate-700 placeholder-slate-400" value={searchValue} onChange={(event) => setSearchValue(event.target.value)} />
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col min-h-125">
        ...
        <div className={'flex-1 flex flex-col items-center justify-center p-10 transition-colors duration-300 ' + (isDragging ? 'bg-asgard-secondary/10' : 'bg-transparent')}
        ...
        >
          ...
          <input type="file" ref={fileInputRef} className="hidden" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" />
        </div>
        ...
        <div className="px-8 py-5 border-t border-slate-100 flex items-center justify-center bg-slate-50/30">
          <div className="flex items-center gap-1 opacity-50 pointer-events-none">
            <button className="px-3 py-1.5 rounded-lg text-slate-400 font-bold">Prev</button>
            ...
            <button className="px-3 py-1.5 rounded-lg text-slate-600 font-bold">Next</button>
          </div>
        </div>
      </div>
    </div>
