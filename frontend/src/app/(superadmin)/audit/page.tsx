  ...
  <TopBar />
   <main className="flex-1 overflow-y-auto p-8 space-y-8">
    ...
    <div className="p-6 border-b-2 border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/50">
     ...
     <input
      type="text"
      placeholder="Cari aksi, entitas, atau ID..."
      value={searchQuery}
   ...
   <div className="overflow-x-auto">
    ...
    <div className="divide-y divide-slate-100">
     {paginatedLogs.map((log) => (
      <div key={log.id} className="px-8 py-5 hover:bg-slate-50/50 transition-colors flex items-start gap-5 border-l-4 border-l-transparent hover:border-l-asgard-secondary">
       <div className="h-10 w-10 shrink-0 bg-slate-100 text-slate-400 rounded-2xl flex items-center justify-center mt-0.5 border-2 border-slate-100">
      {log.action === "LOGIN" ? <User size={18} /> : log.action === "CREATE" ? <FileText size={18} /> : <AlertTriangle size={18} />}
      </div>
