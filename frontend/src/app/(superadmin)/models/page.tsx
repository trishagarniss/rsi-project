...
export default function ModelsPage() {
 ...
 useEffect(() => { if (errorMsg) { const t = setTimeout(() => setErrorMsg(""), 7000); return () => clearTimeout(t); } }, [errorMsg]);
  ...
  const resetForm = () => {
    setFormData({ version: "", algorithm: "", accuracy_score: "", is_active: false });
 ...
 const openDeleteModal = (m: MlModel) => { setSelectedModel(m); setIsDeleteModalOpen(true); };
  ...
  e.preventDefault();
  if (!selectedFile) {
   setErrorMsg("Harap pilih file model (.pkl).");
   return;
  }
  ...
  <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
    ...
    <form onSubmit={handleCreate} className="p-8 space-y-5">
     ...
     <input type="text" required placeholder="Random Forest" value={formData.algorithm} onChange={(e) => setFormData(p => ({ ...p, algorithm: e.target.value }))} className="w-full px-4 h-[46px] rounded-xl border-2 border-slate-200 focus:border-asgard-primary focus:ring-0 outline-none transition-all text-sm font-bold text-slate-800" />
      ...
      <input type="file" accept=".pkl" required onChange={(e) => setSelectedFile(e.target.files?.[0] ?? null)} className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 focus:border-asgard-primary focus:ring-0 outline-none transition-all text-sm font-bold text-slate-800 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-asgard-primary file:text-white file:text-xs file:font-black hover:file:bg-[#2434B5] file:cursor-pointer" />
      <p className="text-[10px] text-slate-400 font-bold mt-1">File yang dipilih: {selectedFile?.name || "-"}</p>
      </div>
     <div className="space-y-1.5">
