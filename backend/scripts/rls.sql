...
CREATE POLICY IF NOT EXISTS tenant_isolation_tenants ON tenants
    USING (id = current_setting('app.tenant_id')::int OR current_setting('app.tenant_id') = '')
    WITH CHECK (id = current_setting('app.tenant_id')::int OR current_setting('app.tenant_id') = '');

CREATE POLICY IF NOT EXISTS tenant_isolation_users ON users
    USING (tenant_id = current_setting('app.tenant_id')::int OR current_setting('app.tenant_id') = '')
    WITH CHECK (tenant_id = current_setting('app.tenant_id')::int OR current_setting('app.tenant_id') = '');
